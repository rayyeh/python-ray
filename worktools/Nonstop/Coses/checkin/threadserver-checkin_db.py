        ...
        while True:
                ...
                if error == 0:
                    ...
                    if result != None:
                        if rule == '0' and result[4] == 1 and error == 0:
                            ...
                            c.execute("select * from pan where (id =? or id =?) and pid = ? order by checkin DESC ",
                                      (id, str.upper(id), F48_PID))
                            result = c.fetchone()
                            print result
                            if result[4] == 1:
            ...
            def reversal(msg):
                ...
                if error == 0:
                    ...
                    #Write tranlog to record 
                    c.execute("insert into tranlog values (?,?,?,?,?,?,?,?,?)",
                              (today, now, F41_TID, card, F48_PID, F39_resp, '', F11_traceno_hex, 1))
                    conn.commit()
                    c.close()
