...
class ISO8583:
	...
	_BITS_VALUE_TYPE[37] = ['37','Retrieval reference number','A',12,'an']
	_BITS_VALUE_TYPE[38] = ['38','Approval code','A',6,'an']
	_BITS_VALUE_TYPE[39] = ['39','Action code','A',2,'an']
	_BITS_VALUE_TYPE[40] = ['40','Service code','A',3,'an']
	_BITS_VALUE_TYPE[41] = ['41','Card acceptor terminal identification','A',8,'ans']
	...
	def __setBitTypeLL(self, bit, value):
		...
		if self._BITS_VALUE_TYPE[bit][4]== 'n':
                    if len(value) % 2 == 1:
                        valuex=value.ljust(len(value)+1,'0')
                    else:
                        valuex=value
                    self.BITMAP_VALUES[bit] = binascii.a2b_hex(size.zfill(2))+binascii.a2b_hex(valuex)
                else:
	...
	def __setBitTypeN(self, bit, value):
		...
		if self.getBitLimit(bit) % 2 == 1:
			self.BITMAP_VALUES[bit] =binascii.a2b_hex(value.zfill(self.getBitLimit(bit)+1))
		else:
			self.BITMAP_VALUES[bit] =binascii.a2b_hex(value.zfill(self.getBitLimit(bit)))
		#print 'setBit ',bit,' value:',binascii.b2a_hex(self.BITMAP_VALUES[bit])
	################################################################################################
	...
	def showIsoBits(self):
		...
		for cont in range(0,128):
			if self.BITMAP_VALUES[cont] <> self._BIT_DEFAULT_VALUE:
				if self._BITS_VALUE_TYPE[cont][4] == 'n' or cont== 63:
				    print("Bit[%s] of type %s has limit %s = %s"%(cont,self.getBitType(cont),self.getBitLimit(cont),binascii.b2a_hex(self.BITMAP_VALUES[cont])))
				else:
	...
	def getRawIso(self):
		...
		resp += binascii.a2b_hex(self.MESSAGE_TYPE_INDICATION)
		resp += binascii.a2b_hex(self.BITMAP_HEX)
	...
	################################################################################################
	#a partir de um trem de string, pega o MTI
	def __setMTIFromStr_POS(self,iso):
		"""Method that get the first 4 characters to be the MTI. 
		It's a internal method, so don't call!
		"""
		
		self.MESSAGE_TYPE_INDICATION = binascii.b2a_hex(iso[0:2])
	...
	################################################################################################
	#Receive a str and interpret it to bits and values
	def __getBitFromStr_POS(self,strWithoutMtiBitmap):
		"""Method that receive a string  without MTI and Bitmaps (first and second), understand it and remove the bits values
		@param: str -> with all bits presents whithout MTI and bitmap
		It's a internal method, so don't call!
                For POS message ONLY,If ASCII ISO8583,Pls use __getBitFromStr()
		"""
			...
			if self.BITMAP_VALUES[cont] <> self._BIT_DEFAULT_VALUE:
				...
				if self.getBitType(cont) == 'LL':
					if self._BITS_VALUE_TYPE[cont][4] == 'n':
						valueSize = int(binascii.b2a_hex(strWithoutMtiBitmap[offset:offset+1]))
						print 'LL:', valueSize
					else:
						valueSize = int(strWithoutMtiBitmap[offset:offset +2])
						print 'LL:', valueSize
					...
					if self._BITS_VALUE_TYPE[cont][4] == 'n':
					    if valueSize % 2 == 1:
							self.BITMAP_VALUES[cont] = strWithoutMtiBitmap[offset:offset+1]  + strWithoutMtiBitmap[offset+1:offset+1+(valueSize+1)/2]                                 
					    else:
							self.BITMAP_VALUES[cont] = strWithoutMtiBitmap[offset:offset+1]  + strWithoutMtiBitmap[offset+1:offset+1+(valueSize/2)]
					else:
						self.BITMAP_VALUES[cont] = strWithoutMtiBitmap[offset:offset+2]  + strWithoutMtiBitmap[offset+2:offset+2+valueSize]
				...
				if self.getBitType(cont) == 'A' or self.getBitType(cont) == 'ANS':
					self.BITMAP_VALUES[cont] = strWithoutMtiBitmap[offset:self.getBitLimit(cont)+offset]
					if self.DEBUG == True:
						print '\tSetting bit %s value %s' % (cont,self.BITMAP_VALUES[cont])					
					offset += self.getBitLimit(cont)
	...
	################################################################################################
	#Parse a POS iso to object
	def setIsoContent_POS(self,iso):
		"""Method that receive a complete ISO8583 string (POS) understand it and remove the bits values
		Example:
			iso = '0210B238000102C080040000000000000002100000000000001700010814465469421614465701081100301000000N399915444303500019991544986020   Value not allowed009000095492'
			i2 = ISO8583()
			# in this case, we need to redefine a bit because default bit 42 is LL and in this especification is "N"
		...
		self.__setMTIFromStr_POS(iso)	# MTI is first 2 byte


		isoT = binascii.b2a_hex(iso[2:]) # Bitmap
		self.__getBitmapFromStr(isoT)		
	...
	def getBitsAndValues(self):
		...
		for cont in range(2,128):
			if self.BITMAP_VALUES[cont] <> self._BIT_DEFAULT_VALUE:
				...
				if self._BITS_VALUE_TYPE[cont][4]== 'n':
					_TMP['value'] = binascii.b2a_hex(self.BITMAP_VALUES[cont])
				else:
	...
	################################################################################################	
	#Method that return ISO8583 to TCPIP network form, with the size in the beginning. By Ray Yeh
	def getNetworkISO_POS(self,tpdu,bigEndian=True):
		"""Method that return ISO8583 POS package with the size in the beginning
		...
		netIso = ""
		asciiIso = binascii.a2b_hex(tpdu)+self.getRawIso()
	...
	################################################################################################	
	# Method that recieve a ISO8583 ASCII package in the network form and parse it. By Ray Yeh
	def setNetworkISO_POS(self,iso, bigEndian=True):
		"""Method that receive sie +POS 8583 package and transfor it in the ISO8583 object.
