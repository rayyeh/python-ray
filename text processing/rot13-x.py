...
class RotateStream(object):
	...
	MARKUP_START = '<'
	MARKUP_END = '>'
	def __init__(self, tags,string_tags):
		self.tags = tags
	...
	def rotate_from_string(self, handle):
		"""
		Rotate from a file handle.
		Takes a file-like object and translates
		text from it into ROT13 text.
...
if __name__ == '__main__':
	...
	"""
		Input from string
	"""
	parser.add_option('-s','--string',dest="string_tags",
		help="Rotate string",default=False,
		action="store_true")
		
	options, args = parser.parse_args()
	rotator = RotateStream(options.tags,options.string_tags)
	if options.tags:
		for letter in rotator.rotate_from_file(sys.stdin):
			sys.stdout.write(letter)
			
	if options.string_tags:		
		for letter in rotator.rotate_from_string(sys.argv[2]):
			sys.stdout.write(letter)